...
import { DealerRetailerList, DealerRetailerListItem, dealerRetailerApi } from "../../services/dealerRetailerApi";
...
export default function DealerRetailersScreen({ onBack }: { onBack: () => void }) {
  ...
  const load = async (refresh = false) => {
    ...
    try {
      const result = await dealerRetailerApi.list(1, search);
  ...
  const loadMore = async () => {
    ...
    setLoadingMore(true);
    try {
      const next = await dealerRetailerApi.list(data.page + 1, search);
      if (currentRequest === requestId.current) {
